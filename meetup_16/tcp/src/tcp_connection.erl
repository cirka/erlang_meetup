...
handle_info({tcp,Socket,<<"quit", _Rest/binary>>}, #state{socket=Socket} = State) ->
    supervisor:start_child(tcp_pool,[]),
    {stop,normal,State};

handle_info({tcp,Socket,<<"lala", _Rest/binary>>}, #state{socket=Socket} = State) ->
    gen_tcp:send(Socket,["pats to lala :-)",10,13]),
    {noreply,State};
